...
#parameters
window_size = args.window_size #size of the sliding window; 0 = use document as window
...
if os.path.isfile(args.ref_corpus_dir):
    corpus_partitions.append(args.ref_corpus_dir)
else:
...
for i, cp in enumerate(corpus_partitions):
    print("creating a thread for corpus partition " + cp + "\n")
    po.apply_async(calcwcngram, (i, window_size, cp, topic_word_rel, unigram_list, unigram_rev,), callback=calcwcngram_complete)
